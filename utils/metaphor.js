...
export async function getURLs(request, breadth) {
  ...
  const metaphorSearchPromptsJSON = JSON.parse(metaphorSearchPrompts.text);

  const queriesPromises = metaphorSearchPromptsJSON.map(async (metaphorSearchPrompt) => {
    const data = {
  ...
  const searchQueries = await Promise.allSettled(queriesPromises);

  const successfulQueries = searchQueries
